<template>
  <aside 
    class="bg-[#1a2538] flex flex-col text-white transition-all duration-300 ease-in-out overflow-hidden flex-shrink-0"
  ...
  >
    <div :class="sidebarStore.isCollapsed ? 'flex items-center justify-center' : 'p-4'">
        <div :class="sidebarStore.isCollapsed ? 'mb-0 mx-auto justify-center w-[40px]' : 'flex items-center gap-3 mb-8'"
          ...
          <div class="relative w-10 h-10 flex items-center justify-center flex-shrink-0 transition-transform duration-300 cursor-pointer group">
            ...
            <div class="absolute top-full left-1/2 -translate-x-1/2 mt-1.5 px-2.5 py-1 bg-[#1e2d44] rounded-md shadow-lg text-[11px] text-gray-300 whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-50">
              {{ sidebarStore.isCollapsed ? '展开菜单' : '收起菜单' }}
              <div class="absolute -top-1 left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-[#1e2d44] rotate-45"></div>
            </div>
          </div>
      ...
      <nav v-if="!sidebarStore.isCollapsed" class="space-y-1">
        <a href="#" class="flex items-center gap-3 rounded-lg transition-colors overflow-hidden"
           :class="[
             ...
             activeMenu === 'chat' ? 'bg-[#2d4059] text-white' : 'text-gray-400 hover:bg-[#243347] hover:text-white'
           ]"
          ...
          <span class="text-sm whitespace-nowrap transition-opacity duration-300"
                :class="sidebarStore.isCollapsed ? 'opacity-0 w-0 overflow-hidden' : 'opacity-100'">New Chat</span>
        </a>
        <a href="#" class="flex items-center gap-3 rounded-lg transition-colors overflow-hidden"
           :class="[
             sidebarStore.isCollapsed ? 'px-3 py-3 justify-center' : 'px-4 py-3',
             activeMenu === 'library' ? 'bg-[#2d4059] text-white' : 'text-gray-400 hover:bg-[#243347] hover:text-white'
           ]"
          ...
          <span class="text-sm whitespace-nowrap transition-opacity duration-300"
                :class="sidebarStore.isCollapsed ? 'opacity-0 w-0 overflow-hidden' : 'opacity-100'">My Library</span>
        </a>
        <a href="#" class="flex items-center gap-3 rounded-lg transition-colors overflow-hidden"
           :class="[
             sidebarStore.isCollapsed ? 'px-3 py-3 justify-center' : 'px-4 py-3',
             activeMenu === 'history' ? 'bg-[#2d4059] text-white' : 'text-gray-400 hover:bg-[#243347] hover:text-white'
           ]"
          ...
          <span class="text-sm whitespace-nowrap transition-opacity duration-300"
                :class="sidebarStore.isCollapsed ? 'opacity-0 w-0 overflow-hidden' : 'opacity-100'">Search History</span>
        </a>
        <a href="#" class="flex items-center gap-3 rounded-lg transition-colors overflow-hidden"
           :class="[
             sidebarStore.isCollapsed ? 'px-3 py-3 justify-center' : 'px-4 py-3',
             activeMenu === 'settings' ? 'bg-[#2d4059] text-white' : 'text-gray-400 hover:bg-[#243347] hover:text-white'
           ]"
          ...
          <span class="text-sm whitespace-nowrap transition-opacity duration-300"
                :class="sidebarStore.isCollapsed ? 'opacity-0 w-0 overflow-hidden' : 'opacity-100'">Settings</span>
        </a>
      </nav>
    </div>
    ...
    <div v-if="!sidebarStore.isCollapsed" class="flex-1 flex flex-col overflow-hidden">
      <template v-if="activeMenu === 'chat'">
        <div class="px-4 pt-4">
          <div class="border-t border-[#2d4059] pt-4 mb-3">
            <div class="flex items-center justify-between px-2">
              <span class="text-xs text-gray-500 uppercase tracking-wider font-medium">Chat History</span>
        ...
        <template v-if="isChatHistoryOpen">
          <div class="px-4 mb-3">
            <div class="flex items-center gap-2 px-3 py-2 bg-[#243347] rounded-lg">
              ...
              <input v-model="chatSearchQuery"
                     ...
                     class="flex-1 bg-transparent text-xs text-gray-300 placeholder-gray-500 outline-none" />
            </div>
          </div>
          ...
          <div class="flex-1 overflow-y-auto px-4 pb-4 space-y-1">
            <div v-for="(chat, index) in filteredChats" :key="index"
                 ...
                 :class="selectedChatIndex === index ? 'bg-[#2d4059]' : 'hover:bg-[#243347]'"
                 @click="handleSelectChat(index)">
              <div class="flex-1 min-w-0 pr-16">
                <div class="text-sm text-white truncate">{{ chat.title }}</div>
                <div class="text-xs text-gray-500 mt-0.5">{{ chat.time }}</div>
              </div>
              <div class="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button class="p-1 text-gray-500 hover:text-blue-400 transition-colors"
                        title="Rename chat"
                ...
                <button class="p-1 text-gray-500 hover:text-red-400 transition-colors"
                        title="Delete chat"
            ...
            <div v-if="filteredChats.length === 0" class="flex flex-col items-center justify-center py-8 text-gray-500">
      ...
      <template v-else-if="activeMenu === 'library'">
        <div class="px-4 pt-4">
          <div class="border-t border-[#2d4059] pt-4 mb-3 px-2">
            <span class="text-xs text-gray-500 uppercase tracking-wider font-medium">Recent Downloads</span>
          </div>
        </div>
        <div class="flex-1 overflow-y-auto px-4 pb-4 space-y-1">
          <div v-for="(paper, index) in recentDownloads" :key="index"
               ...
               :class="selectedDownloadIndex === index ? 'bg-[#2d4059]' : 'hover:bg-[#243347]'"
               @click="handleSelectDownload(index)">
            <div class="flex-1 min-w-0">
              <div class="text-sm text-white truncate">{{ paper.title }}</div>
              <div class="text-xs text-gray-500 mt-0.5">{{ paper.authors }}</div>
            </div>
            <div class="flex-shrink-0">
              ...
              <svg v-else class="w-4 h-4 text-gray-400 animate-spin" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <div v-if="!sidebarStore.isCollapsed" class="px-4 py-3 border-t border-[#2d4059]">
      <a href="#" class="flex items-center gap-3 rounded-lg text-gray-400 hover:bg-[#243347] hover:text-white transition-colors px-4 py-2.5"
...
const emit = defineEmits<{
  (e: 'selectChat', index: number): void
  (e: 'newChat'): void
}>()
