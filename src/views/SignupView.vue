    ...
    <div class="relative w-full max-w-md mx-4">
      <div class="bg-white rounded-2xl shadow-2xl p-8 animate-fade-in">
        <div class="flex flex-col items-center mb-8">
          ...
          <p class="text-sm text-gray-500 mt-1">Create your account to start your research</p>
        </div>
        ...
        <div v-if="successMessage" class="mb-6 p-3 bg-green-50 border border-green-200 rounded-lg flex items-center gap-2">
        ...
        <form @submit.prevent="handleSignup" class="space-y-5">
          ...
          <div class="flex items-start gap-2">
            <input v-model="agreeTerms" type="checkbox" class="w-4 h-4 mt-0.5 rounded border-gray-300 text-[#1a2538] focus:ring-[#1a2538]" />
            <label class="text-sm text-gray-600">
              I agree to the <a href="#" class="text-[#1a2538] hover:underline font-medium">Terms of Service</a> and <a href="#" class="text-[#1a2538] hover:underline font-medium">Privacy Policy</a>
            </label>
          </div>
        ...
        <div class="mt-6 text-center">
          ...
          <a href="#" class="text-sm text-[#1a2538] hover:underline font-medium" @click.prevent="navigateToLogin">Sign In</a>
        </div>
      </div>
    </div>
  </div>
...
function clearError(field: 'username' | 'email' | 'password' | 'confirmPassword') {
  errors[field] = ''
  errorMessage.value = ''
}
    ...
    await new Promise<void>((resolve, reject) => {
      setTimeout(() => {
        ...
        if (existingEmails.includes(email.value.toLowerCase())) {
          ...
          existingEmails.push(email.value.toLowerCase())
