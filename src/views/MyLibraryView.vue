<template>
  <div class="flex h-screen overflow-hidden bg-theme-bg-primary">
    ...
    <div class="flex flex-1 overflow-hidden relative">
      <main class="flex flex-col bg-theme-bg-primary overflow-hidden flex-1 min-w-0">
        <div class="flex items-center justify-between px-6 py-4 border-b border-theme-border-primary bg-theme-bg-card flex-shrink-0">
          <h1 class="text-xl font-bold text-theme-text-primary">{{ $t('library.title') }} <span class="text-lg font-normal text-theme-text-secondary ml-2">{{ $t('library.subtitle') }}</span></h1>
        </div>

        <div class="flex-1 overflow-y-auto">
          <div class="p-6">
            <div class="bg-theme-bg-card rounded-xl border border-theme-border-primary shadow-sm p-6">
              <h2 class="text-2xl font-bold text-theme-text-primary leading-snug mb-4">{{ currentPaper.title }}</h2>

              <div class="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-theme-text-tertiary mb-5 pb-5 border-b border-theme-border-secondary">
                ...
                <div class="flex items-center gap-1.5">
                  ...
                  <span>{{ $t('library.citationsLabel') }}: {{ currentPaper.citations }}</span>
                </div>
              </div>

              <div class="flex flex-wrap gap-2 mb-6">
                <span v-for="(kw, i) in currentPaper.keywords" :key="i"
                      class="px-2.5 py-1 bg-theme-bg-active text-theme-text-secondary text-xs rounded-lg">
                  {{ kw }}
                </span>
              </div>

              <div class="mb-8">
                <h3 class="text-sm font-bold text-theme-text-primary mb-3">{{ $t('library.abstract') }}</h3>
                <p class="text-sm text-theme-text-secondary leading-relaxed">{{ currentPaper.fullAbstract }}</p>
              </div>

              <div class="mb-8">
                <h3 class="text-sm font-bold text-theme-text-primary mb-3">{{ $t('library.modelPerformance') }}</h3>
                <div class="overflow-hidden rounded-lg border border-theme-border-primary">
                  <table class="w-full text-sm">
                    <thead>
                      <tr class="bg-theme-bg-success">
                        <th class="border-b border-r border-theme-border-secondary px-4 py-2.5 text-left font-semibold text-theme-text-primary">{{ $t('library.modelColumnName') }}</th>
                        <th class="border-b border-r border-theme-border-secondary px-4 py-2.5 text-left font-semibold text-theme-text-primary">{{ $t('library.corePerformance') }}</th>
                        <th class="border-b border-r border-theme-border-secondary px-4 py-2.5 text-left font-semibold text-theme-text-primary">{{ $t('library.testDataset') }}</th>
                        <th class="border-b border-theme-border-secondary px-4 py-2.5 text-left font-semibold text-theme-text-primary">{{ $t('library.year') }}</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr v-for="(row, index) in performanceData" :key="index"
                          :class="row.isSota ? 'bg-theme-bg-success' : (index % 2 === 0 ? 'bg-theme-bg-card' : 'bg-theme-bg-input')">
                        <td class="border-b border-r border-theme-border-primary px-4 py-2.5 font-medium" :class="row.isSota ? 'text-theme-text-primary' : 'text-theme-text-secondary'">
                          {{ row.model }}
                        </td>
                        <td class="border-b border-r border-theme-border-primary px-4 py-2.5" :class="row.isSota ? 'font-bold text-theme-text-primary' : 'text-theme-text-secondary'">
                          {{ row.accuracy }}
                        </td>
                        <td class="border-b border-r border-theme-border-primary px-4 py-2.5" :class="row.isSota ? 'font-semibold text-theme-text-primary' : 'text-theme-text-secondary'">
                          {{ row.dataset }}
                        </td>
                        <td class="border-b border-theme-border-primary px-4 py-2.5" :class="row.isSota ? 'font-bold text-theme-text-primary' : 'text-theme-text-secondary'">
                          {{ row.year }}
                        </td>
                      </tr>
                    </tbody>
              ...
              <div class="mb-8">
                <h3 class="text-sm font-bold text-theme-text-primary mb-4">{{ $t('library.modelImplementation') }}</h3>
                <div class="flex gap-8">
                  <div class="flex-1">
                    <div class="bg-theme-bg-active rounded-lg p-6 flex items-center justify-center border border-theme-border-primary">
                      <svg viewBox="0 0 280 120" class="w-full max-w-[280px]">
                        <rect x="10" y="40" width="35" height="40" rx="4" fill="var(--bg-input)" stroke="var(--border-secondary)" stroke-width="1"/>
                        <text x="27" y="65" text-anchor="middle" class="text-[10px]" fill="var(--text-secondary)">Input</text>

                        <line x1="45" y1="60" x2="65" y2="60" stroke="var(--border-secondary)" stroke-width="1"/>

                        <rect x="65" y="30" width="20" height="60" rx="3" fill="var(--bg-input)" stroke="var(--border-secondary)" stroke-width="1"/>
                        <text x="75" y="65" text-anchor="middle" class="text-[8px]" fill="var(--text-secondary)">TF</text>

                        <line x1="85" y1="60" x2="105" y2="60" stroke="var(--border-secondary)" stroke-width="1"/>

                        <rect x="105" y="20" width="70" height="80" rx="6" fill="var(--bg-success)" stroke="var(--border-secondary)" stroke-width="1"/>
                        <text x="140" y="55" text-anchor="middle" class="text-[10px] font-semibold" fill="var(--text-primary)">Model</text>
                        <text x="140" y="75" text-anchor="middle" class="text-[8px]" fill="var(--text-secondary)">Implementation</text>

                        <line x1="175" y1="60" x2="195" y2="60" stroke="var(--border-secondary)" stroke-width="1"/>

                        <rect x="195" y="30" width="20" height="60" rx="3" fill="var(--bg-input)" stroke="var(--border-secondary)" stroke-width="1"/>
                        <text x="205" y="65" text-anchor="middle" class="text-[8px]" fill="var(--text-secondary)">TF</text>

                        <line x1="215" y1="60" x2="235" y2="60" stroke="var(--border-secondary)" stroke-width="1"/>

                        <rect x="235" y="40" width="35" height="40" rx="4" fill="var(--bg-input)" stroke="var(--border-secondary)" stroke-width="1"/>
                        <text x="252" y="65" text-anchor="middle" class="text-[10px]" fill="var(--text-secondary)">Output</text>
                      </svg>
                    </div>
                  </div>
                  ...
                  <div class="flex-1 space-y-4">
                    <div>
                      <h4 class="text-xs font-bold text-theme-text-primary mb-2">{{ $t('library.coreInnovations') }}</h4>
                      <ul class="text-xs text-theme-text-secondary space-y-1.5">
                    ...
                    <div>
                      <h4 class="text-xs font-bold text-theme-text-primary mb-2">{{ $t('library.technicalImplementationDetails') }}</h4>
                      <ul class="text-xs text-theme-text-secondary space-y-1.5">
                        <li class="flex items-start gap-2">
                    ...
                    <div>
                      <h4 class="text-xs font-bold text-theme-text-primary mb-2">{{ $t('library.openSourceCode') }}</h4>
                      <a href="#" class="text-xs text-theme-text-link hover:underline flex items-center gap-1">
      ...
      <aside
        class="flex flex-col bg-theme-bg-card overflow-hidden transition-all duration-300 ease-in-out border-l border-theme-border-primary flex-shrink-0"
      ...
      >
        <template v-if="isRightSidebarOpen">
          <div class="flex items-center justify-between px-4 py-3 border-b border-theme-border-primary flex-shrink-0">
            <h3 class="text-sm font-bold text-theme-text-primary">{{ $t('library.downloadedLiteratureList') }}</h3>
            <button class="p-1 text-theme-text-tertiary hover:text-theme-text-secondary hover:bg-theme-bg-active rounded-lg transition-colors"
          ...
          <div class="px-4 py-3 border-b border-theme-border-primary flex items-center gap-2 flex-shrink-0">
            <div class="flex-1 flex items-center gap-2 px-3 py-2 bg-theme-bg-active rounded-lg border border-transparent focus-within:border-theme-border-focus transition-colors">
              ...
              <input v-model="searchQuery" type="text" :placeholder="$t('library.search')" class="flex-1 bg-transparent text-xs text-theme-text-primary placeholder-theme-text-tertiary outline-none"/>
            </div>
            <button class="px-3 py-2 bg-theme-bg-card border border-theme-border-primary rounded-lg text-xs text-theme-text-secondary hover:bg-theme-bg-hover transition-colors">{{ $t('library.filter') }}</button>
                    <button class="px-3 py-2 bg-theme-bg-card border border-theme-border-primary rounded-lg text-xs text-theme-text-secondary hover:bg-theme-bg-hover transition-colors flex items-center gap-1">
              {{ $t('library.sort') }}
          ...
          <div class="flex-1 overflow-y-auto p-3 space-y-2">
            <div v-for="(paper, index) in filteredPapers" :key="index"
                 class="bg-theme-bg-card rounded-xl border p-3 cursor-pointer transition-all hover:shadow-sm"
                 :class="selectedPaper?.title === paper.title ? 'border-theme-accent-primary shadow-md' : 'border-theme-border-primary'"
                 @click="selectedPaper = paper">
              <h4 class="text-xs font-semibold text-theme-text-primary leading-snug mb-2 line-clamp-2">{{ paper.title }}</h4>
              <div class="flex items-center justify-between mb-2">
                <span class="text-xs text-theme-text-tertiary">{{ paper.authors }} | {{ paper.year }}</span>
              </div>
              <div class="flex items-center justify-between pt-2 border-t border-theme-border-secondary">
                <button class="px-4 py-1.5 bg-theme-btn-primary-bg text-white text-xs rounded-lg hover:bg-theme-btn-primary-hover transition-colors flex items-center gap-1.5"
      ...
      <button
        ...
        class="absolute right-0 top-1/2 -translate-y-1/2 z-10 w-6 h-12 bg-theme-bg-card border border-theme-border-primary rounded-l-lg shadow-sm flex items-center justify-center text-theme-text-tertiary hover:text-theme-text-secondary hover:bg-theme-bg-hover transition-colors"
